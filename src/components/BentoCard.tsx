import { Link } from "react-router-dom";
import { motion } from "framer-motion";
import { Dashboard } from "../assets/assets";

type Report = {
  title: string;
  industrySlug: string;
  slug: string;
  color?: string;
  classname?: string;
  img?: string;
};

const BentoCard: React.FC<Report> = ({
  title,
  industrySlug,
  slug,
  color,
  classname,
  img,
}) => {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.5 }}
      transition={{
        duration: 0.5,
        delay: 0,
        ease: [0, 0.71, 0.2, 1.01],
      }}
      whileInView={{ opacity: 1, scale: 1 }}
      whileHover={{ scale: 0.96 }}
      viewport={{ once: false }}
      className={`relative p-6 group min-h-[400px] rounded-3xl overflow-hidden col-span-1 row-span-1 transition-colors + ${
        classname || ""
      }`}
    >
      <Link
        to={`/reports/${industrySlug}/${slug}`}
        className="block text-[#183B4E] h-full group-hover:text-gray-600 transition-colors"
      >
        <img
          src={img || Dashboard}
          alt=""
          className="absolute top-0 left-0 w-full h-full object-cover z-0 transition-transform duration-500 rounded-3xl group-hover:scale-105"
          style={{ opacity: 1 }}
        />
        {/* Overlay: slide up from bottom on hover */}
        <div
          className={`absolute left-0 backdrop-blur bottom-0 w-full h-0 bg-${
            color ? color : "black"
          }/30 z-10 opacity-100 group-hover:h-full transition-all ease-in-out duration-500 pointer-events-none`}
          style={{ transitionProperty: "height, background-color" }}
        ></div>
        <div className="relative z-20 translate-y-10 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 transition-transform duration-400 ease-in-out  flex flex-col justify-between h-full group">
          <div className="text-h3 font-normal mb-4 text-white">{title}</div>
          <div className="flex text-white translate-y-20 group-hover:translate-y-0 transition-transform duration-400 ease-in-out items-center justify-between text-gray-600">
            <span className="text-sm">Read More →</span>
            <span>{industrySlug}</span>
          </div>
        </div>
      </Link>
    </motion.div>
  );
};

export default BentoCard;
